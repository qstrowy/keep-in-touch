...
These boundaries apply while executing the Module 3 Lesson 2 testing workflow; later lessons and ordinary repository
maintenance may change the named areas through their own approved plans.

- Do not configure hooks, hook lifecycle, or debugging hooks within M3L2. That is Lesson 3.
- Do not configure MCP servers, Playwright API, e2e code, or multimodal scenario code within M3L2. That is Lesson 4.
- Do not run the bug-to-fix-to-regression-test workflow within M3L2. That is Lesson 5.
- Do not author CI/CD pipelines from scratch within M3L2. That is Module 1 Lesson 5 / Module 2 Lesson 5.
- Do not run `/10x-test-plan` to change the risk strategy during M3L2. That is Lesson 1. Use `/10x-test-plan --status` to read current state.
...
- `.agents/prompts/m3l2-ad-hoc-testing.md` — ad-hoc file-level testing prompt
